...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--analysis-spec",
        type=Path,
        help="YAML analysis-layer inventory adjustment specification.",
    )
    parser.add_argument(
        "--adjustments-out",
        type=Path,
        help="JSON audit artifact for the applied analysis adjustments.",
    )
...
def build_reconciliation(
    ...
    inventory_adjustments: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    all_dates = [
        row.get("date")
...
def render_markdown(analysis: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 적용 분석 조정: {len(metadata.get('applied_inventory_adjustments', []))}건",
        ...
        "## 요약",
        "",
        f"- 기초 장부수량(순수량): {summary['opening_stock_quantity']:,} (정상 {summary.get('opening_normal_stock_quantity', 0):,}, 음수 {summary.get('opening_negative_stock_quantity', 0):,})",
        f"- 기초 FIFO 원가층: {summary.get('opening_costed_layer_quantity', 0):,} / {money(summary['opening_stock_amount'])}원",
        ...
        f"- 실제 재고수량 차이 품목: {summary.get('quantity_difference_count', summary['quantity_reconciliation_mismatch_count']):,}개",
        f"- 수량 계산 검증 오류: {summary.get('quantity_validation_error_count', 0):,}건",
        f"- 재고 스냅샷 검증 오류: {summary.get('inventory_snapshot_validation_error_count', 0):,}건",
        f"- 전역 귀속 불가 수량 오류: {summary.get('global_quantity_validation_error_count', 0):,}건",
        f"- 전체 수량 대사 검증 상태: {summary.get('quantity_reconciliation_validation_status', 'valid')}",
...
def main() -> int:
    ...
    try:
        analysis = build_reconciliation(
            ...
            analysis_spec["inventory_adjustments"],
        )
    ...
    adjustments_out = args.adjustments_out or args.base_dir / "analysis_adjustments.json"
    adjustments_out.parent.mkdir(parents=True, exist_ok=True)
    adjustments_out.write_text(
        json.dumps(
            {
                "analysis_spec": analysis_spec,
                "applied_inventory_adjustments": analysis["metadata"]["applied_inventory_adjustments"],
            },
            ensure_ascii=False,
            indent=2,
        ) + "\n",
