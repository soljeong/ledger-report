...
`FIFO계산`은 품목별 매입·매출 흐름과 매출의 매입층 배정 근거를 하나의 연속 표로 전개합니다. Excel에서 FIFO를 재계산하거나 수식으로 복제하지 않고, `inventory_reconciliation.json`의 매입·매출 원가 이벤트와 배정 결과만 표시합니다. 하나의 매출이 여러 매입층에 배정되면 `transaction_quantity`는 각 행에 반복될 수 있으므로, 수량 합계는 `allocated_quantity`, 원가 합계는 `fifo_cost_amount`를 사용합니다. 후속 매입 소급배정·미배정수량·취소 또는 복원은 계산 사실로 표시하며, 취소로 줄어든 미배정수량은 음수 `unallocated_cancellation` 행으로 남깁니다. 이 시트는 판정·경고 보고용 시트가 아닙니다.

`원청별 전표손익`은 원청 합계 → 전표 합계 → 개별 매출의 Excel 행 그룹으로 구성합니다. 매출 공급가액과 이미 계산된 FIFO 매출원가를 같은 행에 놓고, 공급가액 기준 매출총이익과 이익률을 표시합니다. 취소·반품은 원본 부호를 유지하며, 원가가 확정되지 않은 매출은 원가·이익·이익률을 빈 값으로 남깁니다.

`원청별 품목수불`은 원청 합계 → 품목 합계 → FIFO 거래 상세의 Excel 행 그룹으로 구성합니다. 해당 원청의 매출만 표시하지만, 관련 품목의 매입과 기말재고는 품목 전체 금액을 각 원청에 반복 표시합니다. 한 품목이 여러 원청에 공통으로 연결된 경우 하단에서 중복된 기말재고금액만 차감해 전체 기말 FIFO 재고금액과 맞추며, 기말재고수량 표시 합계는 공통 품목이 중복된다는 안내를 함께 표시합니다.
