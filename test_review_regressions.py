...
class ReviewRegressionTests(unittest.TestCase):
    def test_report_uses_summary_values_without_diagnostic_balance_chart(self):
        ...
        summary.update(
            ...
            opening_stock_amount_exact="0",
            period_purchase_cost_amount=100,
            period_purchase_cost_amount_exact="100",
            period_sales_supply_amount=200,
            period_sales_supply_amount_exact="200",
            sales_amount=200,
            inventory_amount_at_fifo=0,
            ending_fifo_inventory_amount=0,
            ending_fifo_inventory_amount_exact="0",
            post_period_backfill_amount=0,
            period_backfilled_amount_exact="0",
            prior_period_shortage_settlement_amount=0,
            prior_period_shortage_settlement_amount_exact="0",
            gross_profit=100,
            gross_profit_exact="100",
        )

        html = render_report_html(sources)

        self.assertIn("200원", html)
        self.assertIn("100원", html)
    ...
    def test_invalid_blank_date_is_excluded(self):
        bad = _sale("")
        self.assertEqual(
            _reconcile([_purchase("2026-01-01")], [bad])["summary"][
                "period_sales_record_count"
            ],
            0,
        )
    ...
    def test_vat_error_sets_reference_error_status(self):
        sale = _sale("2026-01-02")
        sale["total_amount"] += 1
        self.assertEqual(
            _reconcile([_purchase("2026-01-01")], [sale])["summary"][
                "post_vat_reference_status"
            ],
            "error",
        )
    ...
    def test_all_shortages_are_shown_in_unconfirmed_shipments(self):
        later = _sale("2026-01-02")
        later.update(voucher=2, excel_row=2)
    ...
    def test_error_date_does_not_create_principal_group(self):
        sources = deepcopy(load_sources(BASE))
        sources["sales"]["records"].append(_sale("bad-date", product=77))
        self.assertNotIn("bad-date", render_report_html(sources))

    def test_decimal_costs_are_rounded_after_aggregation(self):
        r = _reconcile(
            [_purchase("2026-01-01", quantity=10, price="0.49")],
            [_sale("2026-01-02", quantity=10)],
        )
        self.assertEqual(r["summary"]["fifo_sales_cost_amount"], 5)

    def test_negative_sales_amount_renders_without_diagnostic_panel(self):
        sources = deepcopy(load_sources(BASE))
        ...
        html = render_report_html(sources)
        self.assertIn("매출 공급가액", html)
        self.assertNotIn("금액 밸런스", html)

    def test_negative_purchase_amount_renders_without_diagnostic_panel(self):
        sources = deepcopy(load_sources(BASE))
        sources["reconciliation"]["summary"].update(
            gross_profit_status="confirmed",
            period_purchase_cost_amount=-1,
        )
        html = render_report_html(sources)
        self.assertIn("기간 매입원가", html)
        self.assertNotIn("금액 밸런스", html)
