...
DEFAULT_INPUT = DEFAULT_DATA_DIR / "sales.json"
DEFAULT_OUTPUT = DEFAULT_DATA_DIR / "sales_voucher_groups.xlsx"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export sales rows grouped by date and voucher.")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    return parser.parse_args()


def load_sales(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("metadata", {}).get("transaction_type") != "sales":
        raise ValueError(f"expected sales JSON: {path}")
    return payload


def voucher_key(row: dict[str, Any]) -> str:
    return f"{row['date']}-{row['voucher']}"
...
def group_sales_rows(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def export_workbook(sales: dict[str, Any], output_path: Path) -> None:
    groups = group_sales_rows(sales["records"])
...
def main() -> int:
    ...
    export_workbook(sales, args.output)
